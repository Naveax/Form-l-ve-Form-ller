...
The verifier `scripts/verify_v26_q138_signed_block2_explicit_factor.py` constructs all3584 sparse dyadic product vectors, builds a normalized exact echelon basis of2784 vectors, computes the coordinate map of every natural product channel, and reconstructs all3584 channels exactly.

Coordinate structure:

- maximum nonzeros in one natural-channel coordinate vector:41;
- mean nonzeros: `2.5398995535714284...`;
- every coordinate coefficient is dyadic;
...
Hence the block2 rank2784 reduction is not merely an existential Schmidt-rank statement: it has an explicit finite exact coordinate transform and explicit retained-column basis, both far below the current double-round structural peaks.
