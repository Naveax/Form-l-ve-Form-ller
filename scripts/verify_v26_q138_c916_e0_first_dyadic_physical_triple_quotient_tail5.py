#!/usr/bin/env python3
"""Exact quotient-descent test for one fifth-tail physical ternary target."""
...
import probe_v26_q138_c916_e0_first_dyadic_physical_triple_value_image_tail5 as S
...
TARGET = int(os.environ.get("C916_PHYSICAL_QUOTIENT_TAIL5_TARGET", "0"))
...
def analyze():
    ...
    shortlist, _anchors = S.candidate_triples_tail5(gids, ordered)
    ...
    out = {
        ...
        "decision": (
            "EXACT_TAIL5_PHYSICAL_TERNARY_OBSTRUCTION_DESCENDS_TO_SIGN_REFLECTION_QUOTIENT"
            if qholes
            else (
                "EXACT_TAIL5_PHYSICAL_TERNARY_OBSTRUCTION_IS_SIGN_ONLY_AT_SIGN_REFLECTION_QUOTIENT"
                if raw_holes
                else "EXACT_TAIL5_PHYSICAL_TERNARY_PAIRWISE_COMPLETE_NEGATIVE_CONTROL"
            )
        ),
    }
    output_path = os.environ.get("C916_PHYSICAL_QUOTIENT_TAIL5_OUTPUT")
    if output_path:
        path = Path(output_path)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(out, sort_keys=True) + "\n")
    print("result", json.dumps(out, sort_keys=True), flush=True)
    print("PASS V26_Q138_C916_E0_FIRST_DYADIC_PHYSICAL_TRIPLE_QUOTIENT_TAIL5_EXACT")
