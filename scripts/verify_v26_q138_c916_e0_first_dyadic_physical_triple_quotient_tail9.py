#!/usr/bin/env python3
"""Exact quotient-descent test for one ninth-tail physical ternary target."""
from __future__ import annotations
import json,os,sys
from pathlib import Path
sys.path.insert(0,str(Path(__file__).resolve().parent))
import probe_v26_q138_c916_e0_first_dyadic_physical_triple_value_image as T
import probe_v26_q138_c916_e0_first_dyadic_physical_triple_value_image_tail9 as S
...
def analyze():
    ...
    ordered=T.build_ordered_groups(); shortlist,_=S.candidate_triples_tail9(gids,ordered)
    ...
    out={"position":"C","physical_shared_dimension":T.PHYS_N,"tail9_target":TARGET,"triple":list(triple),
         ...
         "decision":("EXACT_TAIL9_PHYSICAL_TERNARY_OBSTRUCTION_DESCENDS_TO_SIGN_REFLECTION_QUOTIENT" if qholes else
                     ("EXACT_TAIL9_PHYSICAL_TERNARY_OBSTRUCTION_IS_SIGN_ONLY_AT_SIGN_REFLECTION_QUOTIENT" if raw_holes else
                      "EXACT_TAIL9_PHYSICAL_TERNARY_PAIRWISE_COMPLETE_NEGATIVE_CONTROL"))}
    output=os.environ.get("C916_PHYSICAL_QUOTIENT_TAIL9_OUTPUT")
    if output:
        p=Path(output); p.parent.mkdir(parents=True,exist_ok=True); p.write_text(json.dumps(out,sort_keys=True)+"\n")
    print("result",json.dumps(out,sort_keys=True),flush=True)
    print("PASS V26_Q138_C916_E0_FIRST_DYADIC_PHYSICAL_TRIPLE_QUOTIENT_TAIL9_EXACT")
