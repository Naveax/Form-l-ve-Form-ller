#!/usr/bin/env python3
"""Aggregate exact tail7 quotient results against the merged 80-ternary physical inventory."""
...
def analyze():
    files=sorted(SUMMARY_DIR.glob("tail7_quotient_*.json")); assert len(files)==16
    rows=[json.loads(p.read_text()) for p in files]; by={int(r["tail7_target"]):r for r in rows}; assert set(by)==set(range(16))
    ...
    admitted=tuple(T4.TAIL3)+tuple(T4.TAIL4)+t5+t6
    assert len(frozen)==38 and len(admitted)==42
    assert not(set(frozen)&set(admitted))
    current=tuple(sorted(set(frozen)|set(admitted))); assert len(current)==80
    t7=tuple(sorted(tuple(map(int,r["triple"])) for r in promotable))
    duplicate=tuple(sorted(set(t7)&set(current))); new=tuple(sorted(set(t7)-set(current)))
    ...
    out={"position":"C","physical_shared_dimension":149,"tail7_targets":16,
         "tail7_promotable_quotient_factors":len(promotable),"tail7_sign_only_raw_obstructions":len(sign_only),
         "tail7_pairwise_complete_targets":len(pairwise_complete),"tail7_total_quotient_holes":sum(int(r["quotient_holes"]) for r in promotable),
         "promotable_rows":[{"tail7_target":int(r["tail7_target"]),"triple":list(map(int,r["triple"])),"raw_holes":int(r["raw_holes"]),"quotient_holes":int(r["quotient_holes"]),"quotient_hole_digest_sha256":str(r["quotient_hole_digest_sha256"])} for r in promotable],
         ...
         "decision":("TAIL7_EXACT_QUOTIENT_PROMOTION_SCOUT_WITH_MATCHING_TREEWIDTH_CERTIFICATE" if upper==lower else "TAIL7_EXACT_QUOTIENT_PROMOTION_SCOUT_WITH_TREEWIDTH_BOUNDS")}
