#!/usr/bin/env python3
"""Aggregate exact tail6 quotient results against the merged 69-ternary physical inventory."""
...
SUMMARY_DIR = Path(os.environ.get("C916_PHYSICAL_QUOTIENT_TAIL6_SUMMARY_DIR", "tail6-quotient-summaries"))
...
def load_tail5_scopes():
    ...
    assert len(scopes) == 9
    assert sum(int(by[i]["quotient_holes"]) for i in range(16)) == 31
    return scopes


def analyze():
    files = sorted(SUMMARY_DIR.glob("tail6_quotient_*.json"))
    ...
    admitted = tuple(T4.TAIL3) + tuple(T4.TAIL4) + load_tail5_scopes()
    assert len(frozen) == 38 and len(admitted) == 31
    assert not (set(frozen) & set(admitted))
    current = tuple(sorted(set(frozen) | set(admitted)))
    assert len(current) == 69

    tail6_scopes = tuple(sorted(tuple(map(int, r["triple"])) for r in promotable))
    duplicate = tuple(sorted(set(tail6_scopes) & set(current)))
    new = tuple(sorted(set(tail6_scopes) - set(current)))
    ...
    out = {
        ...
        "tail6_targets": 16,
        "tail6_promotable_quotient_factors": len(promotable),
        "tail6_sign_only_raw_obstructions": len(sign_only),
        "tail6_pairwise_complete_targets": len(pairwise_complete),
        "tail6_total_quotient_holes": sum(int(r["quotient_holes"]) for r in promotable),
        "promotable_rows": [{
            "tail6_target": int(r["tail6_target"]),
        ...
        "decision": (
            ...
            if upper == lower else "TAIL6_EXACT_QUOTIENT_PROMOTION_SCOUT_WITH_TREEWIDTH_BOUNDS"
        ),
    }
    print("result", json.dumps(out, sort_keys=True), flush=True)
    print("PASS V26_Q138_C916_E0_FIRST_DYADIC_PHYSICAL_TRIPLE_QUOTIENT_TAIL6_SUMMARY")
    print("boundary=tail6 relations remain candidate additions until exact quotient jobs and aggregate pass; no weighted count is inferred")
    print("ALPHA_PASS=0")
    return out
