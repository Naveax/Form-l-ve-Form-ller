#!/usr/bin/env python3
"""Exact physical-only junction compiler for the 105-ternary inventory through tail8."""
...
import verify_v26_q138_c916_e0_first_dyadic_tail8_expanded_physical_treewidth8_exact as E8

TAIL8_DIR=Path(os.environ.get("C916_TAIL8_QUOTIENT_DIR","authorities/tail8-quotient"))
TAIL7_COUNT=4303642359643745525845765825187273472
...
def load_tail8_factors():
    files=sorted(TAIL8_DIR.glob("tail8_quotient_*.json")); assert len(files)==16
    ...
    for i in range(16):
        ...
        out.append({"name":f"tail8:{i}","scope":tuple(map(int,r["triple"])),
                    "qsizes":tuple(map(int,r["quotient_alphabet_sizes"])),
                    "forbidden":forbidden,"digest":str(r["quotient_hole_digest_sha256"])})
    assert tuple(sorted(f["scope"] for f in out))==E8.EXPECTED_TAIL8_SCOPES
    assert holes==36
    return tuple(out)

def analyze():
    frozen=P.load_ternary_factors(); t3=E3.load_tail3_factors(); t4=E4.load_tail4_factors()
    t5=E5.load_tail5_factors(); t6=E6.load_tail6_factors(); t7=E7.load_tail7_factors(); t8=load_tail8_factors()
    assert (len(frozen),len(t3),len(t4),len(t5),len(t6),len(t7),len(t8))==(38,10,12,9,11,13,12)
    prior=frozen+t3+t4+t5+t6+t7; factors=prior+t8
    scopes=[f["scope"] for f in factors]; assert len(scopes)==105 and len(set(scopes))==105
    ...
    assert tuple(map(tuple,fill_edges))==E8.EXPECTED_FILL_EDGES
    ...
    lift=math.prod(qsizes[v] for v in newvars); lifted=TAIL7_COUNT*lift
    ...
    out={"position":"C","physical_shared_dimension":149,"frozen_ternary_factors":38,
         ...
         "tail7_promoted_ternary_factors":13,"tail8_promoted_ternary_factors":12,"expanded_ternary_factors":105,
         ...
         "exact_tail8_expanded_physical_layer_assignment_count":int(total),
         "exact_tail8_expanded_physical_layer_assignment_log2":math.log2(total),
         "tail7_expanded_physical_layer_assignment_count":TAIL7_COUNT,
         "newly_introduced_variables_vs_tail7":list(newvars),
         "tail7_lift_factor_into_tail8_variable_universe":lift,
         "tail7_lifted_assignment_count_in_tail8_variable_universe":lifted,
         "removed_assignments_vs_lifted_tail7_inventory":lifted-total,
         "gain_vs_lifted_tail7_inventory_log2_bits":math.log2(lifted)-math.log2(total),
         "max_positive_separator_message_rows":max((r["positive_message_rows"] for r in messages),default=1),
         "clique_tables":list(meta),
         "decision":"C916_TAIL8_EXPANDED_PHYSICAL_FACTORS_EXACT_JUNCTION_COUNT"}
    print("result",json.dumps(out,sort_keys=True),flush=True)
    print("PASS V26_Q138_C916_E0_FIRST_DYADIC_TAIL8_EXPANDED_PHYSICAL_JUNCTION_COMPILER_EXACT")
