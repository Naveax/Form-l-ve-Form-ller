#!/usr/bin/env python3
"""Aggregate exact tail5 quotient results against the merged 60-ternary physical inventory."""
...
SUMMARY_DIR = Path(os.environ.get("C916_PHYSICAL_QUOTIENT_TAIL5_SUMMARY_DIR", "tail5-quotient-summaries"))
...
def analyze():
    files = sorted(SUMMARY_DIR.glob("tail5_quotient_*.json"))
    ...
    admitted = tuple(T4.TAIL3) + tuple(T4.TAIL4)
    assert len(frozen) == 38 and len(admitted) == 22
    assert not (set(frozen) & set(admitted))
    current = tuple(sorted(set(frozen) | set(admitted)))
    assert len(current) == 60

    tail5_scopes = tuple(sorted(tuple(map(int, r["triple"])) for r in promotable))
    duplicate = tuple(sorted(set(tail5_scopes) & set(current)))
    new = tuple(sorted(set(tail5_scopes) - set(current)))
    ...
    out = {
        ...
        "tail5_targets": 16,
        "tail5_promotable_quotient_factors": len(promotable),
        "tail5_sign_only_raw_obstructions": len(sign_only),
        "tail5_pairwise_complete_targets": len(pairwise_complete),
        "tail5_total_quotient_holes": sum(int(r["quotient_holes"]) for r in promotable),
        "promotable_rows": [{
            "tail5_target": int(r["tail5_target"]),
        ...
        "decision": (
            "TAIL5_EXACT_QUOTIENT_PROMOTION_SCOUT_WITH_MATCHING_TREEWIDTH_CERTIFICATE"
            if upper == lower
            else "TAIL5_EXACT_QUOTIENT_PROMOTION_SCOUT_WITH_TREEWIDTH_BOUNDS"
        ),
    }
    print("result", json.dumps(out, sort_keys=True), flush=True)
    print("PASS V26_Q138_C916_E0_FIRST_DYADIC_PHYSICAL_TRIPLE_QUOTIENT_TAIL5_SUMMARY")
    print("boundary=tail5 relations remain candidate additions until exact quotient jobs and aggregate pass; no weighted count is inferred")
    print("ALPHA_PASS=0")
    return out
