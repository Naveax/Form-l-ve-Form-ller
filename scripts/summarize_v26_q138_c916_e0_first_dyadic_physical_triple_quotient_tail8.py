#!/usr/bin/env python3
"""Aggregate exact tail8 quotient results against the merged 93-ternary physical inventory."""
...
SUMMARY_DIR=Path(os.environ.get("C916_PHYSICAL_QUOTIENT_TAIL8_SUMMARY_DIR","tail8-quotient-summaries"))
...
def analyze():
    files=sorted(SUMMARY_DIR.glob("tail8_quotient_*.json")); assert len(files)==16
    rows=[json.loads(p.read_text()) for p in files]; by={int(r["tail8_target"]):r for r in rows}; assert set(by)==set(range(16)); rows=[by[i] for i in range(16)]
    ...
    admitted=tuple(T4.TAIL3)+tuple(T4.TAIL4)+t5+t6+t7
    assert len(frozen)==38 and len(admitted)==55 and not(set(frozen)&set(admitted))
    current=tuple(sorted(set(frozen)|set(admitted))); assert len(current)==93
    ...
    out={"position":"C","physical_shared_dimension":149,"tail8_targets":16,
         "tail8_promotable_quotient_factors":len(promotable),"tail8_sign_only_raw_obstructions":len(sign_only),
         "tail8_pairwise_complete_targets":len(pairwise_complete),"tail8_total_quotient_holes":sum(int(r["quotient_holes"]) for r in promotable),
         "promotable_rows":[{"tail8_target":int(r["tail8_target"]),"triple":list(map(int,r["triple"])),"raw_holes":int(r["raw_holes"]),"quotient_holes":int(r["quotient_holes"]),"quotient_hole_digest_sha256":str(r["quotient_hole_digest_sha256"])} for r in promotable],
         ...
         "decision":("TAIL8_EXACT_QUOTIENT_PROMOTION_SCOUT_WITH_MATCHING_TREEWIDTH_CERTIFICATE" if upper==lower else "TAIL8_EXACT_QUOTIENT_PROMOTION_SCOUT_WITH_TREEWIDTH_BOUNDS")}
    print("result",json.dumps(out,sort_keys=True),flush=True)
    print("PASS V26_Q138_C916_E0_FIRST_DYADIC_PHYSICAL_TRIPLE_QUOTIENT_TAIL8_SUMMARY")
    print("boundary=tail8 relations remain candidate additions until exact quotient jobs and aggregate pass; no weighted count is inferred")
    print("ALPHA_PASS=0")
    return out
