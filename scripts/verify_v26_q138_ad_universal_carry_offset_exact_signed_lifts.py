...
# Unknowns used by the symbolic always-row elimination:
# internal U3,V3,U4,V4 -> 0..127, beta ->128..159.
# RHS symbolic bits: const0, predecessor128 ->1..128, zero indicators93 ->129..221.
...
def main():
    ...
    # Exact-signed lift principle: +/-1 have the same mod2 residue and choosing
    # the exact signed valuation aggregate removes that family completely from
    # the next residual. Grouping by common selected offset costs <=one rank per
    # distinct offset. A e2 uses the stronger clean raw-e2 global cover564.
    for s in (1,-1):
        assert (s-1)%2==0
        assert (s-s)//2==0
