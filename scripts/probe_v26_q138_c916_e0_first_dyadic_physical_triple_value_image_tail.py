...
def candidate_triples_tail(gids, ordered):
    ...
    tail = [rec for rec in ranked if tuple(rec[3]) not in tested][:TAIL_LIMIT]
    assert len(tail) == TAIL_LIMIT
    assert not ({tuple(rec[3]) for rec in tail} & tested)
    return tail, anchors


T.candidate_triples = candidate_triples_tail
