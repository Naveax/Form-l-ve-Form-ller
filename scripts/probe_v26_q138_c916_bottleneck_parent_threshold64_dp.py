...
def analyze():
    ...
    # The parent already exists in the global tree and is not itself a new child.
    # Therefore only its children and descendants must satisfy the threshold.
    root_choices = []
    for split in range(LO+1, HI):
        ls = stats(LO, split)
        rs = stats(split, HI)
    ...
    out = {
        ...
        'parent': {'lo': LO, 'hi': HI, 'size': HI-LO},
        'target_bits': TARGET,
        'direct_root_splits_le_64': len(root_choices),
        'threshold64_feasible': witness is not None,
        'witness': witness,
        'witness_max_depth': max_depth if witness is not None else None,
        'witness_edge_count': len(edges),
        'witness_max_safe_bits': max((e['safe_evaluation_bits'] for e in edges), default=None),
        'worst_witness_edges': sorted(edges, key=lambda e: (-e['safe_evaluation_bits'], -e['function_lambda'], -e['size'], e['lo']))[:20],
    ...
    print('PASS V26_Q138_C916_BOTTLENECK_PARENT_THRESHOLD64_DP')
    print('scope=exact contiguous-interval threshold64 feasibility search inside parent [166,250) for the fixed multiplicity-then-function order')
    print('important=parent edge is retained; every child/descendant edge in a witness must independently satisfy safe_evaluation_bits<=64')
    print('decision=if infeasible, fixed-order contiguous local surgery cannot yield width64 and local reordering is required')
    print('ALPHA_PASS=0')
    return out
