...
EXPECTED_TAIL6_FACTOR_COUNT = 11
EXPECTED_TOTAL_FACTOR_COUNT = 80
...
def load_extended_factor_specs(E):
    ...
    tail6 = load_promoted(
        TAIL6_DIR, "tail6_quotient", "tail6_target",
        EXPECTED_TAIL6_SCOPES, len(base) + len(tail3) + len(tail4) + len(tail5),
    )
    ...
    assert len(tail6) == EXPECTED_TAIL6_FACTOR_COUNT
    factors = base + tail3 + tail4 + tail5 + tail6
...
def run_profile(target: int, output: Path) -> dict:
    ...
    try:
        ...
        if int(regression["exact_profile_count"]) == 0:
            ...
            full = run_one(len(factors), "all_current_80_ternary")
    ...
    out = {
        ...
        "tail6_promoted_ternary_factors": EXPECTED_TAIL6_FACTOR_COUNT,
        "total_ternary_factors": EXPECTED_TOTAL_FACTOR_COUNT,
        "regression": regression,
        "all_current": full,
        "decision": "C916_80_TERNARY_WEIGHTED_EXACT_PROFILE_SUMMAND",
    }
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(out, sort_keys=True) + "\n")
    print("profile_result", json.dumps(out, sort_keys=True), flush=True)
    print("PASS V26_Q138_C916_E0_FIRST_DYADIC_80_TERNARY_WEIGHTED_PROFILE_EXACT")
    print("ALPHA_PASS=0")
    return out
...
def aggregate(directory: Path, output: Path) -> dict:
    ...
    out = {
        ...
        "decision": "C916_COMPLETE_AFFINE_PLUS_80_TERNARY_WEIGHTED_COUNT_PROFILE_MATRIX_EXACT",
    }
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(out, sort_keys=True) + "\n")
    print("result", json.dumps(out, sort_keys=True), flush=True)
    print("PASS V26_Q138_C916_E0_FIRST_DYADIC_80_TERNARY_WEIGHTED_PROFILE_MATRIX_EXACT")
