...
def main():
    ...
    for cond,cans in groups.items():
        ...
        M=G.singleton_side_map(cans[0],R,S)
        for can in cans[1:]:
            U=G.singleton_side_map(can,R,S)
    ...
    print('A_condition_groups',len(groups),
          'condition_free_dimension_distribution',dict(sorted(dimdist.items())),
          'right21_singleton_image_dimension_distribution',dict(sorted(imagedist.items())),
          'max_condition_free_dimension',maxdim,
          'total_group_point_incidences',total,flush=True)
    print('PASS PROBE V26_Q138_AD_THIRD_A_CONDITION_DIMENSIONS')
    print('scope=exact A direct-e2 condition/image geometry only; no assembled-rank theorem')
