#!/usr/bin/env python3
"""Exact 16-leaf profile-251 fallback for the prepared 60-ternary weighted model.
...
def run_leaf(parent: int, child: int, output: Path) -> dict:
    ...
    import verify_v26_q138_c916_e0_first_dyadic_60_ternary_weighted_profile_matrix_exact as M

    assert 0 <= parent < STATE_COUNT
    assert 0 <= child < STATE_COUNT
    factors = M.load_extended_factor_specs(E)
    assert len(factors) == M.EXPECTED_TOTAL_FACTOR_COUNT == 60
    ...
    out = {
        ...
        "decision": "C916_60_TERNARY_WEIGHTED_PROFILE_251_SECOND_LEVEL_LEAF_EXACT",
    }
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(out, sort_keys=True) + "\n")
    print("leaf_result", json.dumps(out, sort_keys=True), flush=True)
    print("PASS V26_Q138_C916_E0_FIRST_DYADIC_60_TERNARY_WEIGHTED_PROFILE_251_LEAF_EXACT")
    print("ALPHA_PASS=0")
    return out
...
def aggregate(profile_directory: Path, leaf_directory: Path, output: Path) -> dict:
    ...
    out = {
        ...
        "exact_60_ternary_weighted_count": total,
        ...
        "decision": "C916_COMPLETE_AFFINE_PLUS_60_TERNARY_WEIGHTED_COUNT_WITH_PROFILE251_SECOND_LEVEL_LEAVES_EXACT",
    }
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(out, sort_keys=True) + "\n")
    print("result", json.dumps(out, sort_keys=True), flush=True)
    print("PASS V26_Q138_C916_E0_FIRST_DYADIC_60_TERNARY_WEIGHTED_PROFILE_MATRIX_WITH_SHARDED_251_EXACT")
    print("boundary=exact for the finite 60-ternary physical inventory; it is not a completeness theorem or end-to-end work exponent")
    print("ALPHA_PASS=0")
    return out
