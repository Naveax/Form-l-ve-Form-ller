...
def analyze():
    ...
    shortlist, _anchors = S.candidate_triples_tail7(gids, ordered)
    ...
    out = {
        ...
        "decision": (
            "EXACT_TAIL7_PHYSICAL_TERNARY_OBSTRUCTION_DESCENDS_TO_SIGN_REFLECTION_QUOTIENT"
            if qholes else (
                "EXACT_TAIL7_PHYSICAL_TERNARY_OBSTRUCTION_IS_SIGN_ONLY_AT_SIGN_REFLECTION_QUOTIENT"
                if raw_holes else "EXACT_TAIL7_PHYSICAL_TERNARY_PAIRWISE_COMPLETE_NEGATIVE_CONTROL"
            )
        ),
    }
    output_path = os.environ.get("C916_PHYSICAL_QUOTIENT_TAIL7_OUTPUT")
    if output_path:
