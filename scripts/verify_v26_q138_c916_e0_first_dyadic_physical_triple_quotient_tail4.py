#!/usr/bin/env python3
"""Exact quotient-descent test for one fourth-tail physical ternary target."""
...
import probe_v26_q138_c916_e0_first_dyadic_physical_triple_value_image_tail4 as S
...
TARGET = int(os.environ.get("C916_PHYSICAL_QUOTIENT_TAIL4_TARGET", "0"))
...
def analyze():
    ...
    shortlist, _anchors = S.candidate_triples_tail4(gids, ordered)
    ...
    out = {
        ...
        "decision": (
            "EXACT_TAIL4_PHYSICAL_TERNARY_OBSTRUCTION_DESCENDS_TO_SIGN_REFLECTION_QUOTIENT"
            if qholes
            else (
                "EXACT_TAIL4_PHYSICAL_TERNARY_OBSTRUCTION_IS_SIGN_ONLY_AT_SIGN_REFLECTION_QUOTIENT"
                if raw_holes
                else "EXACT_TAIL4_PHYSICAL_TERNARY_PAIRWISE_COMPLETE_NEGATIVE_CONTROL"
            )
        ),
    }
