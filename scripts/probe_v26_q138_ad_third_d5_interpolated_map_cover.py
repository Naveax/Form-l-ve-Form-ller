...
def main():
    ...
    for cond,cans in groups.items():
        B=G.affine_basis(cond);M=G.singleton_side_map(cans[0],S,R)
        for can in cans[1:]:assert eq_on_condition(B,G.singleton_side_map(can,S,R),M)
        rows.append((cond,M))
    ...
    for cond,M in rows:
        ...
        F=full_map(M)
        data.append((B,U,F,M))
    assert len(data)==8084
    print('D5_compatible_groups',len(data),
    ...
    inside=len(templates);outside=364;global_upper=max(outside,inside)
    assert sum(cluster_sizes)<=8084
    print('D5_interpolated_template_cover_size',inside,
    ...
    print('D_outside_D5_active_group_upper',outside,
          'D_direct_e2_rank_uniform_upper_from_interpolated_cover',global_upper,
          'D_complete_third_rank_sum_upper',min(2048,global_upper+171),flush=True)
    print('PASS PROBE V26_Q138_AD_THIRD_D5_INTERPOLATED_MAP_COVER')
    print('scope=constructive affine-coset interpolation cover for D direct-e2 inside forced D5; inherited e1 correction only added by subadditivity')
