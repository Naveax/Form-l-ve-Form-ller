...
def rank_dense(rows):return Q.rref_piv(rows)[0]

def build_alpha(cert):
    ctx=R.setup(cert);C,E,B,id2,dims,new2old,A64,B107,extA,inter,extB=ctx
    ...
    for pat in pats:
        ...
        supports[pat]=frozenset(S.lin(a[pos[x]] for x in S.I9) for a,v in H[1].items() if v)
    dist=Counter(len(x) for x in supports.values());rows=[[Fraction(int(i in supports[p])) for i in range(512)] for p in pats]
    rr=rank_dense(rows);print('reconstructed_support_rank',rr,'distribution',dict(sorted(dist.items())),flush=True)
    assert dist==S.EXPECTED,(dist,S.EXPECTED);assert rr==12,rr
    bidx=[pats.index(tuple(map(int,s))) for s in S.BASIS_MASKS];piv=[int(s,2) for s in S.PIVOT_I9]
    basis=[rows[i] for i in bidx];Pinv=Q.inv([[basis[r][c] for c in piv] for r in range(12)]);alpha=[]
    for row in rows:
        y=[row[c] for c in piv];a=[sum(y[k]*Pinv[k][j] for k in range(12)) for j in range(12)]
        assert all(x in (Fraction(-1),Fraction(0),Fraction(1)) for x in a);alpha.append(a)
    return pats,alpha

def tt_profile(alpha,perm):
    lookup={tuple(((r>>(5-i))&1) for i in range(6)):alpha[r] for r in range(64)};out=[]
    for k in range(1,7):
        rows=[]
...
def main():
    ...
    pats,alpha=build_alpha(cert);natural=tt_profile(alpha,tuple(range(6)));best=None;bestperm=None;bestprof=None
    ...
    sector={b:rank_dense([alpha[i] for i,p in enumerate(pats) if p[3]==b]) for b in(0,1)}
    print('PASS V26_QR_Q138_SUPPORT_SELECTOR_TT')
    print('mask_order='+','.join(MASK_NAMES));print('selector_rank=12');print('natural_tt_profile='+','.join(map(str,natural)))
    print('best_internal_tt_profile='+','.join(map(str,bestprof)));print('best_bit_order='+','.join(MASK_NAMES[i] for i in bestperm))
    print('best_internal_max='+str(max(bestprof[:-1])));print('u2_8_sector_ranks=0:'+str(sector[0])+',1:'+str(sector[1]))
