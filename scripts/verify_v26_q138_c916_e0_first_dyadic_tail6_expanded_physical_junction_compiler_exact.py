...
import verify_v26_q138_c916_e0_first_dyadic_tail6_expanded_physical_treewidth8_exact as E6
...
def load_tail6_factors():
    ...
    by={int(r["tail6_target"]):r for r in rows}; assert set(by)==set(range(16))
    out=[]
    for i in range(16):
        ...
        out.append({"name":f"tail6:{i}","scope":tuple(map(int,r["triple"])),
                    "qsizes":tuple(map(int,r["quotient_alphabet_sizes"])),
                    "forbidden":forbidden,"digest":str(r["quotient_hole_digest_sha256"])})
    assert tuple(sorted(f["scope"] for f in out))==E6.EXPECTED_TAIL6_SCOPES
    return tuple(out)

def analyze():
    frozen=P.load_ternary_factors(); t3=E3.load_tail3_factors(); t4=E4.load_tail4_factors()
    t5=E5.load_tail5_factors(); t6=load_tail6_factors()
    assert (len(frozen),len(t3),len(t4),len(t5),len(t6))==(38,10,12,9,11)
    prior=frozen+t3+t4+t5; factors=prior+t6
    scopes=[f["scope"] for f in factors]
    assert len(factors)==80 and len(set(scopes))==80
    ...
    out={"position":"C","physical_shared_dimension":149,"frozen_ternary_factors":38,
         ...
         "expanded_ternary_factors":80,"physical_quaternary_factors":5,
         "exact_treewidth":8,"fill_edges":[list(x) for x in fill_edges],
         ...
         "exact_tail6_expanded_physical_layer_assignment_count":int(total),
         "exact_tail6_expanded_physical_layer_assignment_log2":math.log2(total),
         "tail5_expanded_physical_layer_assignment_count":TAIL5_COUNT,
         "newly_introduced_variables_vs_tail5":list(newvars),
         "tail5_lift_factor_into_tail6_variable_universe":lift,
         "tail5_lifted_assignment_count_in_tail6_variable_universe":lifted,
         "removed_assignments_vs_lifted_tail5_inventory":lifted-total,
         "gain_vs_lifted_tail5_inventory_log2_bits":math.log2(lifted)-math.log2(total),
         "max_positive_separator_message_rows":max((r["positive_message_rows"] for r in messages),default=1),
         "clique_tables":list(meta),
         "decision":"C916_TAIL6_EXPANDED_PHYSICAL_FACTORS_EXACT_JUNCTION_COUNT"}
    print("result",json.dumps(out,sort_keys=True),flush=True)
    print("PASS V26_Q138_C916_E0_FIRST_DYADIC_TAIL6_EXPANDED_PHYSICAL_JUNCTION_COMPILER_EXACT")
