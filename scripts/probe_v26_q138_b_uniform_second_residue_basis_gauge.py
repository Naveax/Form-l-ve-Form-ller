...
def main():
    ...
    if not candidates:
        ...
        print('PASS V26_Q138_B_UNIFORM_SECOND_RESIDUE_BASIS_GAUGE_PROBE')
        print('scope=deterministic basis-order gauge search only; no improved uniform bound found')
        return

    lower, name, mr, rows = candidates[0]
    er = exact_rank(rows)
    print('exact_candidate', name, 'mod_ranks', mr, 'exact_rank', er, flush=True)
    assert er >= lower
    if er < BASELINE_QR:
        print('UNIFORM_B_SECOND_LIFT_CANDIDATE', EXPECTED_SUPPORT + er,
              'quotient_rank', er, 'baseline', EXPECTED_SUPPORT + BASELINE_QR, flush=True)
    else:
        print('no_exact_improvement', 'candidate_exact_rank', er, flush=True)

    print('PASS V26_Q138_B_UNIFORM_SECOND_RESIDUE_BASIS_GAUGE_PROBE')
    print('scope=deterministic GF2 basis-order gauge search on the admitted uniform B sign span')
